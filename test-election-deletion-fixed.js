...
async function testElectionDeletion() {
  ...
  try {
    ...
    const currentElections = await pool.query(`
    ...
    const votes = await pool.query(
      'SELECT vote_id, voter_id FROM votes WHERE election_id = $1',
      [testElection.election_id]
    );
    ...
    const finalElections = await pool.query('SELECT COUNT(*) as count FROM elections');
