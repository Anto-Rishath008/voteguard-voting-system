...
// Hardcoded test users for fallback authentication
const FALLBACK_USERS = [
  {
    id: "1",
    email: "voter@voteguard.com",
    password_hash: "$2a$10$N9qo8uLOickgx2ZMRZoMye.Fcs4lMLaDTJdBJCNUj7MxZs2sJa5aK", // voter123
    role: "voter",
  ...
  },
  {
    ...
    password_hash: "$2a$10$N9qo8uLOickgx2ZMRZoMye.Fcs4lMLaDTJdBJCNUj7MxZs2sJa5aK", // admin123 (same hash for simplicity)
    role: "admin",
  ...
  },
  {
    ...
    password_hash: "$2a$10$N9qo8uLOickgx2ZMRZoMye.Fcs4lMLaDTJdBJCNUj7MxZs2sJa5aK", // superadmin123
    role: "super_admin",
...
async function authenticateWithFallback(email: string, password: string) {
  ...
  // For the demo, accept the original passwords
  ...
  const expectedPassword = validPasswords[user.email.toLowerCase()];
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (supabase) {
      try {
        console.log("Attempting Supabase authentication for:", email.toLowerCase());
        ...
        if (!userError && supabaseUser) {
          ...
          console.log("User not found in Supabase database:", userError);
        }
      } catch (error) {
    ...
    if (!user) {
      ...
      user = await authenticateWithFallback(email, password);
      if (user) {
        authMethod = "fallback";
      }
    }
